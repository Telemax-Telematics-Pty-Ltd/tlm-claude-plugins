...
description: >-
  ...
  code-analyst ở nửa đầu /tlm-qa-analyze.
model: opus
...
```bash
bash .claude/scripts/qa-log.sh <TICKET> tlm-qa-analyze <bước>/3 "spec: <đang làm gì>"
```
...
Dùng **cùng bộ nhãn nguồn** với `skill: tlm-qa-checklist-format` (`[AC-xx]`, `[Comment]`,
