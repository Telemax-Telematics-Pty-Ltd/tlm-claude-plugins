...
chỉ chạy khi `CÓ`. `CHƯA CÓ` → người dùng chạy `/tlm-qa-setup` (skill `tlm-qa-e2e-scaffold` dựng);
...
| Đường dẫn project + `Trạng thái` | `bash .claude/scripts/qa-config.sh playwright` | `CHƯA CÓ` → báo chạy `/tlm-qa-setup`. `KHÔNG DÙNG` → không export gì |
...
Bước verify sau deploy (`/tlm-qa-verify-prod`) lọc bằng `grep: /@prod-safe/` và chỉ chạy
...
Bỏ qua:
- API (thuộc skill `tlm-qa-postman-api-test`, không phải Playwright).
...
`tlm-qa-clickup-bug-format` yêu cầu Actual Result cụ thể — message thật, status thật.
