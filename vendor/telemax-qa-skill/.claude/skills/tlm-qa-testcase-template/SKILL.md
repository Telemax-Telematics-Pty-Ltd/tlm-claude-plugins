...
bước SAU `tlm-qa-checklist-format`: checklist ra danh sách "cần test gì", người review
...
- Python 3 + `openpyxl`. **Đừng chạy `pip install openpyxl` trần** — trên macOS
  ...
  Harness dùng venv chuẩn ở `.claude/.venv`, do `/tlm-qa-setup` tạo:
  ```
...
- `--mode status` — **đọc-only**, không ghi gì, không tạo `.bak`. Trả JSON: phân bố
  ...
  Cổng đầu vào của `/tlm-qa-run` dùng đúng lệnh này thay vì tự viết openpyxl tạm — Read
...
2. Chạy:
   ```
   bash .claude/scripts/qa-py.sh .claude/skills/tlm-qa-testcase-template/scripts/build.py --input cases.json --template assets/template.xlsx --output <out.xlsx>
   ```
...
4. Tính lại công thức: `bash .claude/scripts/qa-py.sh .claude/skills/tlm-qa-testcase-template/scripts/recalc.py <out.xlsx> 60`
