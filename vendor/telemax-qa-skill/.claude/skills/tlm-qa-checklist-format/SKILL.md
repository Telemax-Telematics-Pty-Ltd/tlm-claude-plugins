...
(`tlm-qa-common-validate`, `testcase-writer`, cổng `/tlm-qa-write-cases`); mục F không được
mở cửa sau cho nó.
...
(bộ Type này phải trùng khớp với skill `tlm-qa-testcase-template` để bước gen test case
...
<!-- Viết phản hồi vào đây. VD:
  ...
  Còn lại OK.
Lưu file lại rồi chạy /tlm-qa-apply-feedback TLM-XXXX. -->

(để trống cho người review)
```

Cơ chế: người review ghi vào section này rồi lưu file → chạy
`/tlm-qa-apply-feedback TLM-XXXX` → Claude áp các chỉnh sửa vào đúng mục (theo số thứ
