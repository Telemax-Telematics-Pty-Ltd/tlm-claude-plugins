---
name: tlm-qa-e2e-scaffold
description: >-
  Dùng khi chạy /tlm-qa-setup trên một repo mới cài harness, khi qa-config ghi Playwright
...
Skill này lo **phần quyết định và phần khuôn**. Việc hỏi người dùng do `/tlm-qa-setup`
...
| Chỗ | Gắn thế nào |
|---|---|
| `tlm-qa-playwright-export` | sinh spec `.ts` bằng API Playwright |
...
Tóm tắt cho `/tlm-qa-setup` đúng bốn điều: **có Playwright chưa** · **có framework e2e
...
1. **Project `prod` có `grep: /@prod-safe/`** — hàng rào cứng. Thiếu nó thì
   `/tlm-qa-verify-prod` chạy mọi case lên dữ liệu khách hàng thật.
...
**Không tự chạy `npm install`.** Nêu lệnh, chờ người dùng duyệt ở `/tlm-qa-setup`.
