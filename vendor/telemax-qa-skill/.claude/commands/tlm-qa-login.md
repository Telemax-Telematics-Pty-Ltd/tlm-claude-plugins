...
Chạy khi: lần đầu cài harness, hoặc `/tlm-qa-run` báo đang ở trang login (session hết hạn).
...
`/tlm-qa-login` ngay từ đầu. Script tự kiểm `SingletonLock` và báo lỗi rõ nếu vẫn bị chiếm.

**2. Chạy script seed**

```bash
node .claude/scripts/seed-mcp-profile.mjs          # staging
node .claude/scripts/seed-mcp-profile.mjs --prod   # production, khi tôi gõ /tlm-qa-login prod
```
...
profile. Không thấy `PROFILE_SEEDED` thì đừng chạy `/tlm-qa-run` — MCP sẽ vấp lock.
...
Nhắc tôi: session này dùng lại cho các lần `/tlm-qa-run` sau; hết hạn thì chạy lại
`/tlm-qa-login`. Đóng cửa sổ trình duyệt cũng không mất session, nhưng xoá thư mục profile
thì mất.

**Để cửa sổ trình duyệt mở nguyên** — đừng đóng sau khi đăng nhập xong. `/tlm-qa-run` sẽ
dùng lại chính cửa sổ đó, khỏi phải khởi động lại và khỏi chờ SPA tải nguội lần nữa.

Rồi mời tôi chạy `/tlm-qa-run TLM-XXXX`.
