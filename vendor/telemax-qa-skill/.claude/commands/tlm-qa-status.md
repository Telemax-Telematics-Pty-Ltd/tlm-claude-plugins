...
|---|---|---|
| `analyze: done` | `checklist: false` | "Nhật ký nói đã phân tích nhưng **không thấy** `checklist_$1.md`. File bị xoá hay đổi tên? Chạy lại `/tlm-qa-analyze $1`." |
| `write-cases: done` | `testcase_xlsx: []` | "Không thấy file `.xlsx` nào. Chạy lại `/tlm-qa-write-cases $1`." |
...
> `/tlm-qa-run TLM-2901` đang `in_progress` từ 09:14 (hơn 4 tiếng), ghi chú cuối:
> "case 30/45, Round 1". Nhiều khả năng đứt giữa chừng. Chạy lại `/tlm-qa-run TLM-2901`
...
Không ticket nào → nói thẳng **"chưa chạy ticket nào"**, rồi mời `/tlm-qa-analyze TLM-XXXX`.
...
|---|---|
| Chưa có gì | `/tlm-qa-analyze $1` |
| `analyze: done`, checklist có, chưa apply-feedback | Mở checklist review. Có sửa → ghi vào "Phản hồi review" rồi `/tlm-qa-apply-feedback $1`. Không sửa gì → `/tlm-qa-write-cases $1` |
| `apply-feedback: done` | `/tlm-qa-write-cases $1` |
| `write-cases: done`, có `.xlsx` | Review file test case, rồi `/tlm-qa-run $1` |
| `run: in_progress` | `/tlm-qa-run $1` — cổng sẽ đề xuất `RESUME: có` |
| `run: done` | Review sheet **Defects & Follow-ups** (bản LOCAL), rồi `/tlm-qa-file-bugs $1` |
| `file-bugs: done` | Xong chặng staging. Sau khi deploy: `/tlm-qa-verify-prod $1` |
| `verify-prod: done` | Ticket đã đi hết vòng |
| Bất kỳ chặng nào `failed` | Nêu ghi chú lỗi đã lưu, đề xuất chạy lại đúng chặng đó |

Thiếu điều kiện môi trường (không phải trạng thái ticket) → trỏ sang **`/tlm-qa-doctor`**,
đừng đoán.
