...
Áp `skill: tlm-qa-e2e-scaffold`. Nó lo phần dò và phần khuôn; **phần hỏi là của tôi và bạn**
...
| Không phải app web, hoặc tôi chưa muốn dựng | **C** | Ghi `qa-config.md` mục Playwright `Trạng thái | KHÔNG DÙNG`. Nhánh UI sẽ bị skip như nhánh API khi chưa có Postman — **không chặn** `/tlm-qa-run` |
...
- **LibreOffice thiếu** → file Excel vẫn sinh đúng, chỉ là ô Summary trống tới khi mở
  ...
  `sudo apt install libreoffice-calc`. Không có thì `/tlm-qa-write-cases` chạy xong mới
...
- **Postman collection chưa có** ở đường dẫn khai trong `qa-config.md` → đúng như
  ...
  case sắp tới không phủ API, thay vì phát hiện giữa `/tlm-qa-run`.

> Phần chẩn đoán chi tiết (2b, 2c) cũng chạy được riêng bằng **`/tlm-qa-doctor`** —
...
- Xác nhận mục **Ticket** của `.claude/qa-config.md`: tiền tố ticket (`TLM-`) và hai
  ...
  chúng, nên sai thì `/tlm-qa-run` dừng ở cổng 1 với lý do sai hoàn toàn.
...
tiếp theo nên chạy (`/tlm-qa-analyze TLM-XXXX`).
