...
Khác `/tlm-qa-setup`: setup là chặng **cài đặt**, chạy một lần cho repo mới, có bước xin
duyệt rồi chạy lệnh. Doctor là chặng **soi bệnh**, chạy nhiều lần trong đời repo —
mỗi khi thấy triệu chứng lạ. Trước đây `/tlm-qa-run` bảo "chạy `/tlm-qa-setup` để dọn" khi
...
| Connector Drive | có tool Drive nào không — không có thì `/tlm-qa-file-bugs` chỉ bỏ bước upload, không phải lỗi |
...
- **KHÔNG cài gì, KHÔNG sửa file nào** — đó là `/tlm-qa-setup`.
