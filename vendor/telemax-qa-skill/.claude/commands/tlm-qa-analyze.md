...
1. **Ticket ID** — `$1` rỗng thì hỏi. Nếu tôi không đưa mà bạn suy từ tên nhánh
   ...
   **Nếu tôi dán thẳng nội dung spec vào chat thay vì đưa mã ticket** — **DỪNG, chưa
   ...
   >
   > Bạn tự tạo rồi chạy lại `/tlm-qa-analyze TLM-XXXX`, hoặc để tôi tạo giúp từ nội dung
...
4. Lời mời review — nêu **hai nhánh**, đừng mời `/tlm-qa-apply-feedback` vô điều kiện:
   - Mở file. **Có chỗ cần sửa** → ghi vào section **"Phản hồi review"** ở cuối
     (tham chiếu bằng số: `#4 sai — maxlength thật là 100`), lưu, rồi chạy
     `/tlm-qa-apply-feedback $1`.
   - **Đọc thấy ổn, không sửa gì** → chạy thẳng `/tlm-qa-write-cases $1`.
     `/tlm-qa-apply-feedback` trên một section rỗng không làm gì cả, chỉ tốn một lượt.
